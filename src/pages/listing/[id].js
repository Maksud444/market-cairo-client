                ...
                {!isOwner ? (
                  <div className="space-y-2">
                    ...
                    {(listing.whatsappPhone || listing.seller?.phone) && (
                      <a
                        href={`https://wa.me/${(listing.whatsappPhone || listing.seller.phone).replace(/[^0-9]/g, '')}?text=${encodeURIComponent(`Hi! I'm interested in renting:\n\n*${listing.title}*\n\n💰 ${listing.pricePerDay} EGP/day\n\nView: https://mysouqify.com/listing/${listing._id}`)}`}
                        target="_blank"
                  ...
                  {!isOwner ? (
                    <div className="space-y-2">
                      ...
                      {(listing.whatsappPhone || listing.seller.phone) && (
                        <a
                          href={(() => {
                            const loc = listing.location?.area ? `📍 ${listing.location.area}${listing.location?.city ? `, ${listing.location.city}` : ''}\n` : '';
                            const price = listing.price === 0 || listing.isDonation ? '🎁 FREE' : `💰 ${listing.price?.toLocaleString()} EGP`;
                            const msg = `Hi! I'm interested in your listing:\n\n*${listing.title}*\n\n${loc}${price}\n\nView: https://mysouqify.com/listing/${listing._id}`;
                            return `https://wa.me/${(listing.whatsappPhone || listing.seller.phone).replace(/[^0-9]/g, '')}?text=${encodeURIComponent(msg)}`;
                          })()}
                          target="_blank"
                          rel="noopener noreferrer"
